import { getRecentRepeatAnalysis } from "$lib/trailbase/stats";
...
export const load: PageServerLoad = async ({ params }) => {
	try {
		...
		const result = await getRecentRepeatAnalysis(selectedRounds);
		...
		console.error("최근 repeat 통계 데이터 로드 실패:", err);
