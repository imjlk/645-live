...
export const load: PageServerLoad = async ({ params }) => {
	try {
		...
		const numberStatsResponse = await client
			...
			.read(selectedNumber.toString());

		if (!numberStatsResponse) {
			throw new Error(`번호 ${selectedNumber}에 대한 통계를 찾을 수 없습니다.`);
		}

		const numberStats = numberStatsResponse as {
			number: number;
			draw_count: number;
			bonus_count: number;
		...
		const numberDetail = numberDetailsResponse as
			| {
					number: number;
					color: string;
					section: number;
				...
				filters: [
					{
						or: [
							{
								column: "draw_number_1",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
								column: "draw_number_2",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
								column: "draw_number_3",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
								column: "draw_number_4",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
								column: "draw_number_5",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
								column: "draw_number_6",
								op: "equal",
								value: selectedNumber.toString(),
							},
							{
		...
		const recentDraws = drawResultsResponse.records.map((record) => {
			...
			// 개별 번호 컬럼들을 numbers 배열로 변환
			const numbers: number[] = [
				drawRecord.draw_number_1,
				drawRecord.draw_number_2,
				drawRecord.draw_number_3,
				drawRecord.draw_number_4,
				drawRecord.draw_number_5,
				drawRecord.draw_number_6,
			];

			const isBonus = drawRecord.bonus_number === selectedNumber;
		...
		const averageFrequency =
			totalRounds > 0
				? ((numberStats.draw_count / totalRounds) * 100).toFixed(2)
				: "0.00";
		const expectedFrequency =
			totalRounds > 0 ? ((totalRounds * 6) / 45).toFixed(1) : "0.0";
		const deviation =
			totalRounds > 0
				? (numberStats.draw_count - (totalRounds * 6) / 45).toFixed(1)
				: "0.0";
