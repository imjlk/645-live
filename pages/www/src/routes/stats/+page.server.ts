...
function getDominantPattern(records: unknown[], leftKey: string, rightKey: string): string {
	...
	return (
		Array.from(counts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "3:3"
	);
}
...
function buildRecent10Summary(
	...
	const dominantOddEven = getDominantPattern(
		oddEvenStats,
		"odd_count",
		"even_count",
	);
	const dominantHighLow = getDominantPattern(
		highLowStats,
		"low_count",
		"high_count",
	);
...
function buildRecent100Summary(acStats: ACStat[]): string {
	...
	const dominantBucket = Object.entries(distribution).sort(
		(a, b) => b[1] - a[1],
	)[0]?.[0];
	...
	try {
		...
		return {
			...
			summarySnapshots: {
				...
				recent50: buildRecent50Summary(
					oddEven50,
					highLow50,
				),
				recent100: buildRecent100Summary(ac100),
				overall: buildOverallSummary(
					totalRounds,
					topNumberStats,
					bottomNumberStats,
				),
