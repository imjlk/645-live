...
export const GET = async () => {
	const [statsFreshness, numbersFreshness, acFreshness, bonusFreshness] =
		await Promise.all([
			getStatsFreshness([
				{
	...
	const entries = [
		{
			path: "/stats",
			lastmod: toLastMod(
				statsFreshness.lastUpdatedAt,
				statsFreshness.latestDrawDate,
			),
		},
		{
			path: "/",
		...
		},
		{
			path: "/stats/bonus",
			lastmod: toLastMod(
				bonusFreshness.lastUpdatedAt,
				bonusFreshness.latestDrawDate,
			),
		},
		{
			path: "/stats/numbers",
			lastmod: toLastMod(
				numbersFreshness.lastUpdatedAt,
				numbersFreshness.latestDrawDate,
			),
		},
		{
			path: "/stats/ac",
			lastmod: toLastMod(acFreshness.lastUpdatedAt, acFreshness.latestDrawDate),
		},
		{
			path: "/stats/odd-even",
			lastmod: toLastMod(
		...
		...RECENT_SECTIONS.flatMap((section) =>
			RECENT_ROUNDS.map((rounds) => ({
				...
				lastmod: toLastMod(
					section === "ac"
						? acFreshness.lastUpdatedAt
						: statsFreshness.lastUpdatedAt,
					section === "ac"
						? acFreshness.latestDrawDate
						: statsFreshness.latestDrawDate,
				),
			})),
		),
