...
async function cacheFirst(request: Request, config: CacheConfig): Promise<Response> {
  ...
  try {
    const response = await fetchWithTimeout(request, config.networkTimeoutSeconds);
...
async function networkFirst(request: Request, config: CacheConfig): Promise<Response> {
  try {
    const response = await fetchWithTimeout(request, config.networkTimeoutSeconds);
...
async function staleWhileRevalidate(request: Request, config: CacheConfig): Promise<Response> {
  const cache = await caches.open(config.cacheName);
  const cached = await cache.match(request);
...
async function fetchWithTimeout(request: Request, timeoutSeconds: number = 10): Promise<Response> {
...
function createOfflineResponse(request: Request): Response {
  ...
  if (url.pathname.startsWith('/api/') || url.pathname.includes('/trailbase/')) {
...
function getRouteConfig(url: string): CacheConfig {
  ...
  // Static assets
  if (CACHE_STRATEGIES.STATIC_ASSETS.some(asset => pathname.startsWith(asset))) {
    return {
      strategy: 'cacheFirst',
  ...
  // Images
  if (CACHE_STRATEGIES.IMAGE_PATTERNS.some(pattern => pattern.test(pathname))) {
    return {
      strategy: 'cacheFirst',
