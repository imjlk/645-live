  ...
  return withErrorHandling(
    async (items: TInput[]): Promise<BatchResult<TOutput>> => {
      ...
      if (options.enableLogging) {
        console.log(`🚀 Starting batch processing: ${items.length} items, ${Math.ceil(items.length / options.batchSize)} batches`);
      }
...
async function processBatchWithSemaphore<TInput, TOutput>(
  ...
  try {
    ...
    try {
      ...
      if (options.enableLogging) {
        console.log(`✅ Batch ${batchIndex + 1} completed in ${batchTime.toFixed(2)}ms`);
      }
      ...
      if (options.memoryThreshold && performance.memory) {
        ...
        if (memoryUsage > options.memoryThreshold) {
          if (options.enableLogging) {
            console.log(`🧹 Memory threshold reached (${(memoryUsage / 1024 / 1024).toFixed(2)}MB), suggesting GC`);
          }
...
export class DatabaseBatchProcessor<TInput, TOutput> {
  ...
  async processWithTransactions(items: TInput[]): Promise<BatchResult<TOutput>> {
    if (this.transactionCallback) {
      ...
      const transactionProcessor = createBatchProcessor(wrappedProcessor, this.processor.options);
...
export class StreamBatchProcessor<TInput, TOutput> {
  ...
  private async processBatch(batch: TInput[], totalProcessed: number): Promise<BatchResult<TOutput>> {
    const startTime = performance.now();
    ...
    try {
      ...
      if (this.options.onProgress) {
        this.options.onProgress(totalProcessed + batch.length, totalProcessed + batch.length, Math.floor(totalProcessed / this.options.batchSize) + 1);
      }
...
export class PriorityBatchProcessor<TInput, TOutput> {
  ...
  add(items: TInput[], priority: number = 0): void {
      ...
      while (this.queues.size > 0) {
        ...
        try {
          ...
          if (this.options.enableLogging) {
            console.log(`✅ Processed priority ${highestPriority} batch: ${batch.length} items`);
          }
        } catch (error) {
          const appError = error instanceof Error ? error : new Error(String(error));
