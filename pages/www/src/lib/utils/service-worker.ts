...
class ServiceWorkerManager extends EventTarget {
	...
	private setupRegistrationListeners(
		...
		if (registration.waiting) {
			...
			this.dispatchEvent(
				new CustomEvent("sw-waiting", { detail: registration }),
			);
		}
		...
		registration.addEventListener("updatefound", () => {
			...
			if (newWorker) {
				...
				this.dispatchEvent(
					new CustomEvent("sw-installing", { detail: registration }),
				);
			}
		});
	}

	private trackServiceWorker(
		...
		worker.addEventListener("statechange", () => {
			...
			if (worker.state === "installed") {
				if (navigator.serviceWorker.controller) {
					...
					this.dispatchEvent(
						new CustomEvent("sw-updated", { detail: this.state.registration! }),
					);
				} else {
					// First install
