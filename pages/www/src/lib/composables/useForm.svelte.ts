...
export const validationRules = {
  ...
  pattern: (pattern: RegExp, message = '형식이 올바르지 않습니다.'): ValidationRule<string> => {
    return (value: string): string | null => {
      if (typeof value !== 'string' || !value) return null;
...
export function useForm<T extends Record<string, unknown>>(
  ...
  const { fields: fieldConfigs, validateOnChange = false, onSubmit, onReset } = config;

  // Initialize state
  const initialValues = Object.fromEntries(
    Object.entries(fieldConfigs).map(([key, config]) => [key, config.initialValue])
  ) as T;
  ...
  let fieldValidations = $state<Record<keyof T, ValidationResult>>(
    Object.fromEntries(
      Object.keys(fieldConfigs).map(key => [key, { isValid: true, errors: [] }])
    ) as Record<keyof T, ValidationResult>
  );
  let fieldValidating = $state<Set<keyof T>>(new Set());

  // Validate a single field
  const validateField = async <K extends keyof T>(field: K): Promise<ValidationResult> => {
...
export function useFieldValidation<T>(
  ...
  { setValue: (value: T) => void; validate: () => Promise<ValidationResult>; touch: () => void }
] {
