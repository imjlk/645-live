...
export function createRoundNumber(value: number): RoundNumber {
  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(`Invalid round number: ${value}. Must be a positive integer.`);
  }
...
export function createLottoNumber(value: number): LottoNumber {
  if (!Number.isInteger(value) || value < 1 || value > 45) {
    throw new Error(`Invalid lotto number: ${value}. Must be between 1 and 45.`);
  }
  return value as LottoNumber;
}

/** Six main lotto numbers */
export type LottoNumbers = readonly [LottoNumber, LottoNumber, LottoNumber, LottoNumber, LottoNumber, LottoNumber];
...
export interface ColorDistribution extends Record<BallColor, {
...
/** Section analysis */
export interface SectionAnalysis extends Record<`section_${NumberSection}`, {
  readonly count: number;
  readonly average: number;
  readonly percentage: number;
}> {}
...
export type EventHandler<T extends keyof CustomEvents> = (event: CustomEvent<CustomEvents[T]>) => void;
