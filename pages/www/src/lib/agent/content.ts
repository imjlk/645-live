...
export function getComparePageContent(): AgentPage {
	return {
		...
		description:
			"645.live는 단순 당첨 확인을 넘어 공식 결과 스냅샷, TrailBase 기반 스캔 흐름, 깊이 있는 통계, 기계 판독용 연동 정보을 한 제품 안에 함께 담고 있습니다.",
		intro: [
...
export function getStatusPageContent(): AgentPage {
	return {
		key: "status",
		eyebrow: "상태",
		title: "645.live 서비스 상태",
		description:
			"이 페이지는 645.live의 공개 조회 기능이 현재 어떤 상태인지와, 개발자 연동 관점에서 어떤 용도로 쓰이는지 설명합니다.",
		intro: [
			"공개 조회 기능에는 최근 회차 스냅샷 API, 통계 개요 API, 마크다운 협상 문서, RFC 9727 API 카탈로그, 같은 도메인 MCP 탐색 엔드포인트가 포함됩니다. 회원 스캔 쓰기 작업은 계속 인증이 필요하며 사용자의 Better Auth 세션 상태에 따라 동작합니다.",
		],
		sections: [
			{
				title: "확인하는 서비스",
