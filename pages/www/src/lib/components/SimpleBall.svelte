...
const ballColorClass = $derived(isWinning ? 'bg-gradient-to-br from-yellow-300 to-yellow-500 text-yellow-900 ring-2 ring-yellow-400' : getBallColor(number));
const bonusClass = $derived(isBonus ? 'ring-2 ring-orange-400 ring-offset-2' : '');
