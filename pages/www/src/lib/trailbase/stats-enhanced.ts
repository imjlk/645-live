...
export const getNumberStatistics = withErrorHandling(
  async (options: {
    ...
    const response = await client.records('lotto_number_stats').list(queryOptions);

    return response.records.map((record: Record<string, unknown>): NumberStatistics => {
      ...
      return {
        ...
        averageFrequency: Number.parseFloat(String(record.average_frequency || '0')),
        deviation: Number.parseFloat(String(record.deviation || '0')),
        lastDrawRound: record.last_draw_round ? createRoundNumber(Number(record.last_draw_round)) : null,
        frequency: totalCount > 0 ? (totalCount / (drawCount + bonusCount)) * 100 : 0,
      };
    });
  },
  'getNumberStatistics'
...
export const getOddEvenAnalysis = withErrorHandling(
  async (limit = 10): Promise<readonly OddEvenStatistics[]> => {
    ...
    return response.records.map((record: Record<string, unknown>): OddEvenStatistics => {
...
export const getColorAnalysis = withErrorHandling(
  async (limit = 10): Promise<readonly ColorStatistics[]> => {
    ...
    return response.records.map((record: Record<string, unknown>): ColorStatistics => {
      ...
      const complexity = maxCount <= 1 ? 'low' : maxCount >= 4 ? 'high' : 'medium';
      
      const mostFrequentColor = Object.entries(distribution).reduce((maxEntry, [color, count]) => {
        const [, maxCount] = maxEntry;
        return count > maxCount ? [color, count] : maxEntry;
      }, ['yellow', distribution.yellow])[0] as BallColor;
...
export const getPairStatistics = withErrorHandling(
  async (limit = 20): Promise<readonly PairStatistics[]> => {
    ...
    return response.records.map((record: Record<string, unknown>): PairStatistics => ({
      ...
      lastAppearance: record.last_appearance ? createRoundNumber(Number(record.last_appearance)) : null,
    }));
  },
  'getPairStatistics'
);
...
export function analyzeNumberFrequency(stats: readonly NumberStatistics[]): {
  ...
  return {
    ...
    coldNumbers: stats.filter(s => s.frequency < averageFrequency - threshold),
    averageFrequency,
    standardDeviation,
  };
}
...
export function analyzeSectionBalance(stats: readonly SectionStatistics[]): {
  ...
  const variance = sectionCounts.reduce((sum, count) => sum + (count - idealCount) ** 2, 0) / sectionCounts.length;
...
export function validateLottoNumbers(numbers: readonly unknown[]): numbers is readonly LottoNumber[] {
...
export function createSafeLottoNumbers(numbers: readonly unknown[]): readonly LottoNumber[] | null {
...
export const getComprehensiveRoundStats = withErrorHandling(
  async (_round: RoundNumber): Promise<{
