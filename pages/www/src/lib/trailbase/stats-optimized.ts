...
import {
	...
	useBatchProcessor,
	useMemo,
} from "$lib/composables/useMemoization.svelte.ts";
import type {
	ColorStatistics,
	LottoNumber,
	NumberStatistics,
...
export const getMemoizedFrequencyAnalysis = useAsyncMemo(
	async (numbers: readonly NumberStatistics[]) => {
		console.log(
			"🧮 Calculating frequency analysis for",
			numbers.length,
			"numbers",
		);
		...
		const variance =
			frequencies.reduce(
				(sum, freq) => sum + (freq - averageFrequency) ** 2,
				0,
			) / frequencies.length;
		...
		const trends = {
			...
			stable: numbers
				.filter(
					(n) => n.frequency >= coldThreshold && n.frequency <= hotThreshold,
				)
				.slice(0, 10),
		};
...
export const getMemoizedPatternAnalysis = useMemo(
	(oddEvenStats: readonly OddEvenStatistics[], rounds = 50) => {
		console.log(
			"📊 Calculating pattern analysis for",
			oddEvenStats.length,
			"records",
		);
		...
		// Trend analysis
		const recentBalance =
			balanceScores.slice(0, 10).reduce((sum, score) => sum + score, 0) /
			Math.min(10, balanceScores.length);
		const olderBalance =
			balanceScores.slice(10, 20).reduce((sum, score) => sum + score, 0) /
			Math.min(10, balanceScores.slice(10, 20).length);

		const balanceTrend =
			recentBalance < olderBalance
				? "improving"
				: recentBalance > olderBalance
					? "declining"
					: "stable";
		...
		const mostCommonPattern = Object.entries(patterns).sort(
			([, a], [, b]) => b - a,
		)[0];

		const predictions = {
			nextOdds: mostCommonPattern
				? Number.parseInt(mostCommonPattern[0].split(":")[0]) / 6
				: 0.5,
			confidence: mostCommonPattern
				? mostCommonPattern[1] / recentStats.length
				: 0,
		};
...
export const getMemoizedColorComplexity = useMemo(
	(colorStats: readonly ColorStatistics[], rounds = 30) => {
		console.log(
			"🎨 Calculating color complexity for",
			colorStats.length,
			"records",
		);
		...
		const recentComplexity =
			complexityScores.slice(0, 10).reduce((sum, score) => sum + score, 0) /
			Math.min(10, complexityScores.length);
		const olderComplexity =
			complexityScores.slice(10, 20).reduce((sum, score) => sum + score, 0) /
			Math.min(10, complexityScores.slice(10, 20).length);

		const complexityTrend =
			recentComplexity > olderComplexity
				? "increasing"
				: recentComplexity < olderComplexity
					? "decreasing"
					: "stable";
		...
		const dominantColor = Object.entries(colorTotals).sort(
			([, a], [, b]) => b - a,
		)[0];

		const totalBalls = Object.values(colorTotals).reduce(
			(sum, count) => sum + count,
			0,
		);
		...
		const averageDiversity =
			diversityScores.reduce((sum, score) => sum + score, 0) /
			diversityScores.length;
		const diversityRating =
			averageDiversity > 0.8
				? "high"
		...
		return {
			...
			dominance: {
				color: dominantColor[0] as keyof typeof colorTotals,
...
export const getOptimizedStats = useAsyncMemo(
	async (
		...
		try {
			...
			if (includePatterns) {
				promises.push(
					getOddEvenAnalysis(rounds).then((oddEven) => {
						...
						if (getMemoizedPatternAnalysis.cache.has(oddEven, rounds))
							cacheHits++;
			...
			if (includeColors) {
				promises.push(
					getColorAnalysis(rounds).then((colors) => {
						...
						if (getMemoizedColorComplexity.cache.has(colors, rounds))
							cacheHits++;
			...
			console.log(
				"✅ Optimized stats calculation completed:",
				results.metadata,
			);
			return results;
		} catch (error) {
