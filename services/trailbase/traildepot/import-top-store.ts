	...
	try {
		...
		for (const item of list) {
			...
			stores.push({
				...
				selection_type:
					winType === "1등"
						? normalizeSelectionType(item.atmtPsvYnTxt)
						: undefined,
			});
		}

		const firstCount = stores.filter(
			(store) => store.win_type === "1등",
		).length;
		const secondCount = stores.filter(
...
function getWinningStoreCount(round: number): number {
	const database = getDatabase();
	const row = database
		.prepare(
			"SELECT COUNT(*) as count FROM lotto_winning_stores WHERE round = ?",
		)
...
export async function importWinningStoresForRound(
	round: number,
	...
	const existingCount = getWinningStoreCount(round);
	if (!refresh && existingCount > 0) {
		console.log(
			`⏭️ 회차 ${round} 당첨점이 이미 ${existingCount}개 있어 건너뜁니다. (--refresh로 강제 재수집 가능)`,
		);
		return "skipped";
	}

	const stores = await fetchWinningStores(round);
	return saveWinningStores(round, stores) ? "imported" : "failed";
}
...
export async function importWinningStoresForRange(
	...
	for (let round = startRound; round <= endRound; round++) {
		const result = await importWinningStoresForRound(round, options);
	...
	console.log("\n📊 완료 요약:");
	console.log(`   ✅ 신규 수집: ${importedCount}회차`);
	console.log(`   ⏭️ 스킵: ${skippedCount}회차`);
	console.log(`   ❌ 실패: ${failCount}회차`);
	console.log(`   📈 총 처리: ${endRound - startRound + 1}회차`);
}

export async function updateLatestWinningStores(
	options: ImportOptions = {},
): Promise<void> {
	const database = getDatabase();
	...
	if (!row.latest_round) {
		console.error(
			"❌ lotto_draw_results에 회차 데이터가 없어 당첨점 업데이트를 진행할 수 없습니다.",
		);
		lastFailCount = 1;
		return;
	}
	...
	const result = await importWinningStoresForRound(row.latest_round, options);
	lastFailCount = result === "failed" ? 1 : 0;
}
...
export async function updateLatestWinningStoresRange(
	...
	await importWinningStoresForRange(startRound, latestRound, options);
}
...
명령어:
  single <회차> [--refresh]                    # 특정 회차 당첨점 가져오기
  range <시작회차> <종료회차> [--refresh]       # 범위 회차 당첨점 가져오기
  latest [--refresh]                          # lotto_draw_results 최신 회차 당첨점 가져오기
  latest-range [개수] [--refresh]              # lotto_draw_results 최신 N회차 당첨점 가져오기 (기본값: 10)

기본 동작:
  이미 당첨점 데이터가 있는 회차는 건너뜁니다.
  강제 재수집이 필요하면 --refresh 옵션을 사용하세요.
`);
}
...
async function main(): Promise<void> {
	...
	const positionalArgs = args.slice(1).filter((arg) => arg !== "--refresh");
	...
	try {
		switch (command) {
			case "single": {
				...
				const round = Number.parseInt(positionalArgs[0], 10);
				if (!Number.isInteger(round) || round <= 0) {
					throw new Error("유효한 회차 번호를 입력해주세요.");
				}

				const result = await importWinningStoresForRound(round, { refresh });
				lastFailCount = result === "failed" ? 1 : 0;
				break;
			}
			...
			case "range": {
				...
				const startRound = Number.parseInt(positionalArgs[0], 10);
				const endRound = Number.parseInt(positionalArgs[1], 10);
				...
				await importWinningStoresForRange(startRound, endRound, { refresh });
				break;
			}

			case "latest": {
				await updateLatestWinningStores({ refresh });
				break;
			}
			...
			case "latest-range": {
				const latestRounds =
					positionalArgs.length >= 1
						? Number.parseInt(positionalArgs[0], 10)
						: 10;
				...
				await updateLatestWinningStoresRange(latestRounds, { refresh });
				break;
			}
