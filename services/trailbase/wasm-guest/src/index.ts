...
import { HttpError, HttpHandler, HttpRequest, HttpResponse, StatusCode } from "trailbase-wasm/http";
...
function jsonWithStatus(status: number, payload: Record<string, unknown>): HttpResponse {
  return HttpResponse
    .status(status, JSON.stringify(payload))
...
async function heartbeatHandler(req: HttpRequest): Promise<HttpResponse> {
  ...
  try {
    await query("DELETE FROM active_connections WHERE session_id = ?", [session_id]);
    await query(
...
export default defineConfig({
  ...
  jobHandlers: [
    new JobHandler("Lotto Weekly Updater", "0 40 11 * * 7", async () => {
      await executeLottoUpdate();
    }),
    new JobHandler("Lotto Store Weekly Updater", "0 0 12 * * 7", async () => {
